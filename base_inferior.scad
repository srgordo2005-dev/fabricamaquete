/* ==========================================================================
   PROJETO CAMPO TÁTIL - GABINETE INFERIOR COM UNIÃO MEIA-MADEIRA (LAP JOINT)
   Gera Metade Frontal ou Traseira de 175x175mm com encaixe perfeito de 3mm
   Código Limpo de Acentuação (100% Compatível com OpenSCAD)
   ========================================================================== */

/* [Modo de Visualização / Impressão] */
// Selecione qual parte você deseja gerar para fatiar
modo_peca = "frontal"; // [frontal: Metade Frontal (175x175mm), traseira: Metade Traseira (175x175mm), completo: Caixa Completa (Apenas Visualização)]

/* [Parâmetros Mecânicos Físicos] */
largura_total = 175;      
...
// Carcaça base completa sem corte
...
// Execução do split inteligente da base
if (modo_peca == "frontal") {
    // Renderiza apenas a metade frontal com o degrau macho/fêmea
    translate([0, comprimento_total/4, 0])
        intersection() {
            carcaca_base_completa();
            mascara_corte_frontal();
        }
} else if (modo_peca == "traseira") {
    // Renderiza apenas a metade traseira com o rebaixo complementar
    translate([0, -comprimento_total/4, 0])
        difference() {
            carcaca_base_completa();
            mascara_corte_frontal();
    ...
    // Renderiza a base completa unificada para visualização
    carcaca_base_completa();
}
